...
class ConnectPage(GridLayout):
    ...
    def join_button(self, instance):
        ...
        open('symmetric_keys.txt', 'w').close()
        open('public_keys.txt', 'w').close()
        ...
        Clock.schedule_once(self.generate_keys, 1)
        Clock.schedule_once(self.connect, 1)

    def generate_keys(self, _):
        key_length = 16
        private_key = number.getPrime(key_length)
        public_key = number.getPrime(key_length)
    ...
    def connect(self, _):
        ...
        if not socket_client.connect(ip, port, username, public_key, show_error):
            return
...
class ChatPage(GridLayout):
    def __init__(self, **kwargs):
        ...
        self.friend_username = TextInput(width=Window.size[0]*0.8, height=Window.size[1]*0.2, size_hint_x=None, multiline=False)
        self.encrypt = Button(text="Encrypt")
        self.encrypt.bind(on_press=self.encrypt_message)
        bottom_line1 = GridLayout(cols=2)
        bottom_line1.add_widget(self.friend_username)
        bottom_line1.add_widget(self.encrypt)
        ...
        self.new_message = TextInput(width=Window.size[0]*0.8, height=Window.size[1]*0.2, size_hint_x=None, multiline=False)
    ...
    def encrypt_message(self, _):
        ...
        friend_username = self.friend_username.text
        message = friend_username
        flag = "q"
        socket_client.send(flag, message)
    ...
    def send_message(self, _):
        friend_username = self.friend_username.text
        raw_message = self.new_message.text
        ...
        if friend_username in username_symmetric_key:
            ...
            message = friend_username + ":" + encription.encrypt_message(raw_message, symmetric_key)
            flag = "e"
        ...
        if message:
            ...
            self.history.update_chat_history(f'[color=dd2020]{chat_app.connect_page.username.text}[/color] > {raw_message}')
            socket_client.send(flag, message)
